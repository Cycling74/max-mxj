...
public abstract class InputHandler extends KeyAdapter
{
    ...
    public static final ActionListener KILL = new kill();
      public static final ActionListener YANK = new yank();
	// Default action
	...
	//toph..we have one killring for all open mxj instances
	private static final StringBuffer killring  = new StringBuffer(256);
	private static int lastcaret = 0;
	
	public static class kill implements ActionListener
	{
		public void actionPerformed(ActionEvent evt)
		{
			JEditTextArea textArea = getTextArea(evt);
			...
			try
			{
				if(caret == lastOfLine)
				{
					String s = textArea.getText(caret,1);
					textArea.getDocument().remove(caret,1);
					killring.append(s);
				}
				else
				{
					String s = textArea.getText(caret,lastOfLine - caret);
					textArea.getDocument().remove(caret,lastOfLine - caret);
					killring.append(s);
				}
			}
			catch(BadLocationException bl)
	...
	public static class yank implements ActionListener
	{
		public void actionPerformed(ActionEvent evt)
		{
			JEditTextArea textArea = getTextArea(evt);
			int caret = textArea.getCaretPosition();
			textArea.setSelectedText(killring.toString());
		}
 	}						
